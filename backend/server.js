...
// Registrar movimiento de inventario (usando función RPC atómica)

app.post('/api/inventario/:id/movimiento', 
  authenticate, 
  authorize('stock.entry'),
  auditLog('create', 'movimientos'),
  async (req, res) => {
  ...
  try {
    ...
    // Usar función RPC para movimiento atómico
    ...
    // Verificar si hubo error en la función RPC
    if (data && data.error) {
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    // Verificar contraseña (soporta tanto hasheadas como en texto plano para migración)
    ...
    if (usuario.password.startsWith('$2b$') || usuario.password.startsWith('$2a$')) {
      ...
      // Si es válida, actualizar a hash
      if (passwordValida) {
        const saltRounds = 10;
        const passwordHash = await bcrypt.hash(password, saltRounds);
        await supabase
